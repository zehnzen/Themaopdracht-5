	...
	playerR{ sf::Color::Red, false }
{
	...
	makePlayfield();
	makeLevel();
	unitHandler.setTerrainVector(&terrainContainer);
...
void Game::switchPlayer() {
	...
	if (unitHandler.getUnitSelected()) { 
		unitHandler.setUnitSelected(false);
		std::unique_ptr<Unit>* unit = unitHandler.getUnitPointer();
		markField((*unit)->getWalklimit(), (*unit)->getAttackrange(), true, (*unit)->getTilePosition(), sf::Color::White);
	}
...
void Game::HUD() {
	...
	if (unitHandler.getUnitSelected()) {
		...
		if (unitHandler.getAllySelected()) {
			units = &(playerB.getActive() ? unitBContainer : unitRContainer);
			text.setString(units->at(unitHandler.getUnitIndex())->getName() + ":");
			window.draw(text);
			text.setString("HP:" + std::to_string(units->at(unitHandler.getUnitIndex())->getHP()));
			text.setPosition(ScreenWidth - 140, 130);
			window.draw(text);
			text.setString("DP: " + std::to_string(units->at(unitHandler.getUnitIndex())->getDP()));
			...
			text.setString(units->at(unitHandler.getEnemyIndex())->getName() + ":");
			window.draw(text);
			text.setString("HP:" + std::to_string(units->at(unitHandler.getEnemyIndex())->getHP()));
			text.setPosition(ScreenWidth - 140, 130);
			window.draw(text);
			text.setString("DP: " + std::to_string(units->at(unitHandler.getEnemyIndex())->getDP()));
			text.setPosition(ScreenWidth - 140, 160);
		}
		window.draw(text);
	}
